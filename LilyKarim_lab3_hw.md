...
#The snake species with the smallest average homerange in m^2 is the bitis schneideri commonly known as the namaqua dwarf adder. 
# [according to](https://www.africansnakebiteinstitute.com/snake/namaqua-dwarf-adder/) this snake is about 20 centimeters long and is the smallest adder in the world. It can be found in Namaqualand and Namibia. It is most active in evenings and is vaguely poisonous though likely not fatal.
...
snake <- 
  homerange %>% 
  select(taxon, family,genus, species, common.name, mean.hra.m2) %>% 
  filter(taxon == "snakes") %>% 
  arrange(mean.hra.m2)
print(snake)
```
...
#because the dataframe does not include taxon, make a new data frame containing only birds that can be selected from
...
#the lowest average mass greater than or equal to 4 grams is 5.15 g
...
#the genus and species of the bird with the lowest mean mass (above 4.0g) is regulus regulus. Its common name is the Goldcrest. [I found this information at](https://www.rspb.org.uk/birds-and-wildlife/wildlife-guides/bird-a-z/goldcrest/)
...
ostrich <- 
  birds %>% 
  select(common.name,log10.hra, log10.mass) %>% 
  mutate(hra.mass.ratio = log10.hra/log10.mass) %>% 
