...
ui <- dashboardPage(
  ...
  selectInput("select", "Select", choices = c("Campus", "Academic_Yr"))
  ), # close the first box
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    UC_admit %>% 
      filter(Category == input$category & select == input$select) %>%
      ggplot(aes_string(x = "Ethnicity", y = "FilteredCountFR")) + geom_bar(stat="identity") + theme_light(base_size = 10)
  })
...
ui <- dashboardPage(
  ...
  selectInput("category", "Choose Category", choices = (UC_admit$Category))
  ), # close the first box
  box(title = "Plot ", width = 7,
  plotOutput("plot", width = "600px", height = "500px")
  ) # close the second box
...
server <- function(input, output, session) { 
  ...
  output$plot <- renderPlot({
    UC_admit %>% 
      filter(Academic_Yr == input$year & Campus == input$campus & Category == input$Category) %>%
