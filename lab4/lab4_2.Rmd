...
can use select(variable = "Varaible")
```{r}
tb_data %>% 
...
```{r}
msleep %>% 
  summarize(minbw = min(bodywt),maxbw = max(bodywt), manbw = mean(bodywt))
```
...
msleep %>% 
  group_by(order) %>% 
  summarize(meanbw = mean(brainwt))
```


2. What does `NA` mean?
NA means that there is no data for this variable and order

3. Try running the code again, but this time add `na.rm=TRUE`. What is the problem with Cetacea?
cetacea arewhales, their brains are not measured (its different than missing data)

```{r}
msleep %>% 
  group_by(order) %>% 
  summarize(meanbw = mean(brainwt, na.rm = TRUE)) 
```
