...
It's a numeric and it will modify data because its treated as data
...
```{r}
import_export %>% 
  summarise(number_NA = sum(is.na(import_export)))
```
...
There are no NAs in the data. This is not likely so NA is stored in some other form. 
```{r}
is.na(life_history)
```
